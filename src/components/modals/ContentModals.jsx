    ...
    <ModalShell title="The Process Notebook" accent="coral" onClose={onClose} wide>
      ...
      <ol className="mt-6 space-y-4">
        {processSteps.map((step, i) => (
          ...
          >
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-coral text-sm font-extrabold text-white">
              {i + 1}
            </span>
  ...
  return (
    <ModalShell title="The Trophy Shelf" accent="sun" onClose={onClose} wide>
      ...
      <div className="mt-4 flex justify-end">
        <HiddenCat id={7} size={22} color="#e8a91d" />
      </div>
    </ModalShell>
  )
}
  ...
  return (
    <ModalShell title="Coffee with Ankita" accent="lavender" onClose={onClose}>
      ...
      >
        ...
        <p className="text-[15px] leading-relaxed text-ink">{identity.about}</p>
        <p className="mt-3 text-[15px] leading-relaxed text-ink">{identity.aboutExtra}</p>
        <p className="mt-3 text-[15px] leading-relaxed text-ink-soft">{identity.personality}</p>
      </motion.div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {identity.tags.map((tag, i) => (
          <motion.span
            ...
            className={`rounded-full px-3.5 py-1.5 text-xs font-extrabold text-ink ${
      ...
      <h3 className="mt-6 text-xs font-extrabold tracking-widest text-ink-soft uppercase">Daily toolkit</h3>
...
export function PassportModal({ onClose }) {
  return (
    <ModalShell title="Passport & Postcards" accent="mint" onClose={onClose}>
      <p className="text-[15px] leading-relaxed text-ink">{travel.intro}</p>
        ...
        {travel.stamps.map((s, i) => (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-extrabold tracking-wide uppercase">{s.place}</h3>
            </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <HiddenCat id={8} size={22} color="#3fae90" />
      </div>
    </ModalShell>
  )
}
        ...
        {playMode.lessons.map((l, i) => (
          ...
          >
            <div className="flex items-center gap-2.5">
              <span
                className={`grid h-7 w-7 place-items-center rounded-full text-xs font-extrabold text-ink ${
              ...
              <h3 className="text-sm font-extrabold">{l.title}</h3>
            </div>
    ...
    <ModalShell title="My Journey, in Chapters" accent="wood" onClose={onClose} wide>
        ...
        {chapters.map((c, i) => {
          ...
          return (
            <button
              ...
              onClick={() => setOpen(i)}
              className={`cursor-pointer rounded-lg px-3.5 py-4 text-xs font-extrabold transition-all ${
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <p className="mt-2 text-[15px] leading-relaxed text-ink">{chapters[open].body}</p>
        </motion.div>
      </AnimatePresence>
      <div className="mt-4 flex justify-end">
          ...
          >
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {item.cta && (
                  <a
                    href={`mailto:${identity.email}?subject=Résumé%20request%20—%20found%20the%20drawer`}
                    className="rounded-full bg-coral px-3 py-1 text-[11px] font-extrabold text-white transition hover:bg-coral-deep"
                  >
  ...
  return (
    <ModalShell title="Send a Paper Plane" accent="coral" onClose={onClose}>
      ...
      <motion.a
        ...
        whileTap={{ scale: 0.98 }}
        className="mt-5 inline-flex items-center gap-2.5 rounded-full bg-coral px-7 py-3.5 text-base font-extrabold text-white shadow-[0_10px_24px_-8px_rgba(232,93,61,0.7)] transition-colors hover:bg-coral-deep"
      >
        {contact.cta}
      ...
      <div className="mt-6 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        {identity.links.map((l) => (
          ...
          >
            ...
            <span className="text-ink-soft transition-transform group-hover:translate-x-1" aria-hidden="true">→</span>
          </a>
        ))}
      </div>
      ...
      >
        ...
        <div className="mt-2 flex justify-center">
          <HiddenCat id={11} size={24} color="#6f6a61" />
        </div>
      </motion.div>
    </ModalShell>
  )
